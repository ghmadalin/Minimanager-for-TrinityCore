...
function browse_chars(&$sqlr, &$sqlc)
{
    ...
    if(isset($_GET['search_value']) && isset($_GET['search_by']))
    {
        ...
        $sql_query = "SELECT characters.guid, characters.name, characters.account, characters.race, characters.class, characters.zone, characters.map, characters.totalKills AS highest_rank, characters.online, characters.level, characters.gender, characters.logout_time, COALESCE(guild_member.guildid,0) AS gname FROM characters LEFT JOIN guild_member ON characters.guid = guild_member.guid WHERE $where_out ORDER BY $order_by $order_dir LIMIT $start, $itemperpage";
    ...
    else
    {
        ...
        $query = $sqlc->query("SELECT C.guid,C.name,C.account,C.race,C.class,C.zone,C.map,C.totalKills AS highest_rank,C.online,C.level, C.gender, C.logout_time,COALESCE(guild_member.guildid,0) as gname FROM characters C LEFT JOIN guild_member ON guild_member.guid = C.guid ORDER BY $order_by $order_dir LIMIT $start, $itemperpage");
    }
