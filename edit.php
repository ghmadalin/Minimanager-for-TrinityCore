...
function edit_user(&$sqlr, &$sqlc)
{
    ...
    $refguid = $sqlm->result($sqlm->query('SELECT InvitedBy FROM mmftc_point_system_invites WHERE PlayersAccount = \''.$user_id.'\''), 0, 'InvitedBy');
...
function doupdate_referral($referredby, &$sqlr, &$sqlc)
{
    ...
    if (NULL == $sqlm->result($sqlm->query('SELECT InvitedBy FROM mmftc_point_system_invites WHERE PlayersAccount = \''.$user_id.'\''), 0))
    {
