...
function browse_tickets(&$sqlc)
{
    ...
    $output .="New Tickets: <b>" .$new_record. "</b> | Total Tickets: <b>" .$all_record. "</b>";
    $output .="
                            </td>
    ...
    $output .="
                            <th width=\"5%\"><a href=\"ticket.php?order_by=guid&amp;start=$start&amp;dir=$dir\">".($order_by=='guid' ? "<img src=\"img/arr_".($dir ? "up" : "dw").".gif\" alt=\"\" /> " : "")."{$lang_ticket['id']}</a></th>
                            <th width=\"5%\"><a href=\"ticket.php?order_by=online&amp;start=$start&amp;dir=$dir\">".($order_by=='online' ? "<img src=\"img/arr_".($dir ? "up" : "dw").".gif\" alt=\"\" /> " : "")."Online?</a></th>
                            <th width=\"15%\"><a href=\"ticket.php?order_by=playerGuid&amp;start=$start&amp;dir=$dir\">".($order_by=='playerGuid' ? "<img src=\"img/arr_".($dir ? "up" : "dw").".gif\" alt=\"\" /> " : "")."{$lang_ticket['sender']}</a></th>";
    $output .="
                            <th width=\"50%\">{$lang_ticket['ticket_text']}</th>
                            <th width=\"15%\">{$lang_ticket['ticket_status']}</th>
                        </tr>";
    while ($ticket = $sqlc->fetch_row($query))
    {
        ...
        $output .="
                            <td>$ticket[0]</td>
                            <td>".($ticket[5] ? "<img src=\"img/up.gif\" alt=\"online\">" : "<img src=\"img/down.gif\" alt=\"offline\">")."</td>
                            <td><a href=\"char.php?id=$ticket[1]\">".htmlentities($ticket[4])."</a></td>
                            <td>".htmlentities($ticket[3])." ...</td>";
        if($ticket[2] < 1)
        {
          $output .="
                            <td>{$lang_ticket['ticket_new']}</td>
                        </tr>";
        }
        else
        {
        	$output .="
        	                  <td>{$lang_ticket['ticket_closed']}</td>
                        </tr>";
        }
    }
    unset($query);
