...
class SQL //MySQL Lite
{
    ...
    function connect($db_host, $db_username, $db_password, $db_name, $use_names = '', $pconnect = true, $newlink = false) {
        ...
        if (!file_exists($db_name)){
            @touch($db_name);
            @chmod($db_name, 0666);
            if (!file_exists($db_name)) die($lang_global['err_sql_conn_db']);
        }

        if ((!is_readable($db_name))||(!is_writable($db_name))) die($lang_global['err_sql_conn_db']);
    ...
    function query($sql){
        ...
        if ($this->query_result){
            ...
            die(@sqlite_error_string(sqlite_last_error($this->link_id)));
            return false;
        }
    }
